...
export default {
  ...
  computed: {
    ...
    getUnitId() {
      return this.$store.state.teamState.selectedNavObj.id
    }
  },
  mounted() {
  ...
  methods: {
    ...
    handleManage() {
      ...
      console.log("sur unitId", this.$store.state.teamState.selectedNavObj.id)
      },
    fetchDataRess() {
      axios
        .get('/api/ecran10/list')
        .then(response => {
          const transformData = this.transformData(response.data)
          this.ressource.value = transformData
          console.log('Données récupérées:', this.ressource.value)
        })
        .catch(error => {
